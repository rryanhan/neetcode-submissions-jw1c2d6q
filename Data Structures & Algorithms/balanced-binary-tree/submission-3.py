...
class Solution:
    def isBalanced(self, root: Optional[TreeNode]) -> bool:
        def dfs(root):
            if not root:
                return [True, 0]
            if abs(dfs(root.left)[1] - dfs(root.right)[1]) > 1 or dfs(root.left)[0] == False or dfs(root.right)[0] == False:
                return [False, 1 + max(dfs(root.left)[1], dfs(root.right)[1])]
            else:
                return [True, 1 + max(dfs(root.left)[1], dfs(root.right)[1])]
